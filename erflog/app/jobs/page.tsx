...
interface Job {
  id: string;
  score: number;          // Hybrid score (used for sorting only)
  match_percentage: number; // Normalized semantic score (0-1), shown to user
  title: string;
  company: string;
  description: string;
  link: string;
      ...
      try {
        ...
        const transformed = (data.jobs || []).map(
          (job: TodayDataItem, index: number) => ({
            ...
            match_percentage: job.match_percentage ?? job.score,
            title: job.title,
            company: job.company,
            ...
            source: job.source,
            // Use 80% threshold on match_percentage for high match
            status:
              (job.match_percentage ?? job.score) >= 0.8
                ? "Ready"
                : (job.match_percentage ?? job.score) >= 0.4
                ? "Gap Detected"
                : "Low Match",
            action: (job.match_percentage ?? job.score) >= 0.8 ? "Apply Now" : "View Roadmap",
            tier: (job.match_percentage ?? job.score) >= 0.8 ? "A" : (job.match_percentage ?? job.score) >= 0.4 ? "B" : "C",
            ...
            needs_improvement: job.needs_improvement ?? (job.match_percentage ?? job.score) < 0.8,
          })
        );
        setJobs(transformed);
        ...
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => (
            <JobCard
              ...
              jobTitle={job.title}
              matchScore={Math.round(job.match_percentage * 100)}
              description={job.description}
              link={job.link}
