...
export default function ProblemSolvingPage() {
  ...
  const [currentStep, setCurrentStep] = useState<"search" | "quiz" | "results">(
    "search",
  );
  ...
  // Backend data
  const [categories, setCategories] = useState<LeetCodeCategory[]>([]);
  const [allProblems, setAllProblems] = useState<LeetCodeProblem[]>([]);
  const [isSavingProgress, setIsSavingProgress] = useState(false);

  // Track if user has existing progress (to skip quiz)
  ...
  useEffect(() => {
    const loadProblems = async () => {
      try {
        ...
        const allProbs: LeetCodeProblem[] = [];
        data.categories.forEach((cat) => allProbs.push(...cat.problems));
  ...
  useEffect(() => {
    const loadProgress = async () => {
      ...
      if (isAuthenticated) {
        try {
          const data = await getLeetCodeProgress();
          const hasSolvedProblems =
            data.solved_problem_ids && data.solved_problem_ids.length > 0;
          const hasQuizAnswers =
            data.quiz_answers && Object.keys(data.quiz_answers).length > 0;

          setSolvedProblems(new Set(data.solved_problem_ids || []));
          if (hasQuizAnswers) {
            setQuizAnswers(data.quiz_answers as QuizAnswers);
          }

          // If user has any progress, skip quiz and go directly to results
  ...
  const saveProgressToBackend = useCallback(
    async (solved: number[], quiz?: QuizAnswers) => {
      if (!isAuthenticated) return;
      setIsSavingProgress(true);
      try {
        await saveLeetCodeProgress(solved, quiz as Record<string, string>);
      } catch (error) {
  ...
  // Toggle solved and save to backend
  ...
  const setQuizAnswer = (category: string, level: SkillLevel) => {
    setQuizAnswers((prev) => {
      ...
      // Save to backend with quiz answers
      saveProgressToBackend([...solvedProblems], newAnswers);
      return newAnswers;
    });
    ...
    try {
      ...
      try {
        const submissionsData =
          await leetcodeAPI.getRecentSubmissions(username);
  ...
  const getAIRecommendations = async () => {
    ...
    try {
      const response = await getLeetCodeRecommendations({
        quiz_answers: quizAnswers as Record<string, string>,
        leetcode_profile: profile
          ? {
              total_solved: getTotalSolved(),
              easy_solved: getEasySolved(),
              medium_solved: getMediumSolved(),
              hard_solved: getHardSolved(),
            }
          : undefined,
        solved_problem_ids: [...solvedProblems],
      });

      setAiRecommendedIds(response.recommended_ids || []);
      setRecommendationSource(response.source || "local");
      setCurrentStep("results");
    } catch (error) {
  ...
  const getLocalRecommendations = (): number[] => {
    const unsolved = allProblems.filter((p) => !solvedProblems.has(p.id));
  ...
  const getTotalSolved = () => {
    ...
    return (
      profile.submitStats.acSubmissionNum.find((s) => s.difficulty === "All")
        ?.count || 0
    );
  };
  ...
  const getEasySolved = () => {
    if (!profile?.submitStats?.acSubmissionNum) return 0;
    return (
      profile.submitStats.acSubmissionNum.find((s) => s.difficulty === "Easy")
        ?.count || 0
    );
  };
  ...
  const getMediumSolved = () => {
    if (!profile?.submitStats?.acSubmissionNum) return 0;
    return (
      profile.submitStats.acSubmissionNum.find((s) => s.difficulty === "Medium")
        ?.count || 0
    );
  };
  ...
  const getHardSolved = () => {
    if (!profile?.submitStats?.acSubmissionNum) return 0;
    return (
      profile.submitStats.acSubmissionNum.find((s) => s.difficulty === "Hard")
        ?.count || 0
    );
  };
  ...
  const recommendedProblems = useMemo(() => {
    if (showFullList) return allProblems;
    if (aiRecommendedIds.length > 0) {
      return allProblems.filter((p) => aiRecommendedIds.includes(p.id));
    }
    return allProblems.slice(0, 30);
  }, [aiRecommendedIds, showFullList, allProblems]);

  // Group problems by category
  const groupedProblems = useMemo(() => {
    const groups = new Map<string, LeetCodeProblem[]>();
    recommendedProblems.forEach((problem) => {
      const category = categories.find((c) =>
        c.problems.some((p) => p.id === problem.id),
      );
      if (category) {
    ...
    return groups;
  }, [recommendedProblems, categories]);

  // Blind 75 Progress
  const totalProblemsCount = allProblems.length;
  const blind75Progress = solvedProblems.size;
  const blind75Percentage =
    totalProblemsCount > 0
      ? Math.round((blind75Progress / totalProblemsCount) * 100)
      : 0;
  ...
  const quizAnsweredCount = Object.values(quizAnswers).filter(
    (v) => v !== null,
  ).length;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            {currentStep === "search" &&
              "Get AI-powered Blind 75 recommendations"}
            {currentStep === "quiz" &&
              "Swipe through topics and rate your confidence"}
        ...
        {/* Step 1: Search - Only show if no existing progress */}
        {!isLoadingProgress && currentStep === "search" && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
                ...
                <button
                  ...
                  style={{
                    backgroundColor: quizAnswers[categories[idx].name]
                      ? quizAnswers[categories[idx].name] === "weak"
                        ? "#DC2626"
                        : quizAnswers[categories[idx].name] === "okay"
                          ? "#D97706"
                          : "#16A34A"
                      : idx === currentCardIndex
                        ? "#D95D39"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 mb-8">
                      ...
                      >
                        {(() => {
                          const IconComponent =
                            categoryIcons[currentCategory.icon] || Layers;
                          return (
                            <IconComponent
                              className="w-10 h-10"
                      ...
                      <div>
                        ...
                        <p className="text-gray-600 font-medium">
                          {currentCategory.problems.length} problems
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() =>
                          setQuizAnswer(currentCategory.name, "weak")
                        }
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() =>
                          setQuizAnswer(currentCategory.name, "okay")
                        }
                        className={`w-full flex items-center justify-center gap-3 py-4 rounded-xl transition-all ${
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.98 }}
                        onClick={() =>
                          setQuizAnswer(currentCategory.name, "strong")
                        }
                        className={`w-full flex items-center justify-center gap-3 py-4 rounded-xl transition-all ${
                        ...
                        style={{
                          backgroundColor:
                            quizAnswers[currentCategory.name] === "strong"
                              ...
                              : "#F0FDF4",
                          color: "#16A34A",
                          ...(quizAnswers[currentCategory.name] ===
                            "strong" && { ringColor: "#16A34A" }),
                        }}
                      >
            ...
            <div className="flex items-center justify-between gap-4 max-w-md mx-auto">
              <button
                ...
                className="min-w-[120px] px-6 py-3 rounded-xl font-semibold transition-all disabled:opacity-30 text-gray-700 hover:bg-gray-200"
              ...
              {currentCardIndex < categories.length - 1 ? (
                <button
                  ...
                  className="min-w-[120px] px-6 py-3 rounded-xl font-semibold text-white transition-all hover:opacity-90"
                ...
                <button
                  ...
                  className="min-w-[120px] px-6 py-3 rounded-xl font-semibold text-white transition-all hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-2"
                  style={{
                    backgroundColor: canProceed ? "#D95D39" : "#9CA3AF",
                  }}
                >
                  {isGenerating ? (
                    <>
        ...
        {/* Step 3: Results - Works with or without profile */}
        {!isLoadingProgress && currentStep === "results" && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="space-y-8"
          >
            {/* Profile Header - Only show if profile was fetched */}
            {profile && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                    ...
                    <div className="flex items-center gap-4 mt-2 justify-center md:justify-start">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm text-ink">
                          Rank #
                          {profile.profile?.ranking?.toLocaleString() || "N/A"}
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Target
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-right">
                  <p
                    className="text-3xl font-serif-bold"
                    style={{ color: "#D95D39" }}
                  >
                    {blind75Progress}/{totalProblemsCount}
                  </p>
                  <p className="text-sm text-secondary">
                    {blind75Percentage}% Complete
                  </p>
                </div>
              </div>
                  ...
                  <p className="text-lg font-bold" style={{ color: "#16A34A" }}>
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) =>
                          p.difficulty === "Easy" && solvedProblems.has(p.id),
                      ).length
                    }
                    /
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) => p.difficulty === "Easy",
                      ).length
                    }
                  </p>
                  ...
                  <p className="text-lg font-bold" style={{ color: "#D97706" }}>
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) =>
                          p.difficulty === "Medium" && solvedProblems.has(p.id),
                      ).length
                    }
                    /
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) => p.difficulty === "Medium",
                      ).length
                    }
                  </p>
                  ...
                  <p className="text-lg font-bold" style={{ color: "#DC2626" }}>
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) =>
                          p.difficulty === "Hard" && solvedProblems.has(p.id),
                      ).length
                    }
                    /
                    {
                      allProblems.filter(
                        (p: LeetCodeProblem) => p.difficulty === "Hard",
                      ).length
                    }
                  </p>
              ...
              <div className="flex justify-center mb-6">
                <button
                  ...
                  style={{
                    backgroundColor: showFullList ? "#D95D39" : "#F0EFE9",
                    color: showFullList ? "#fff" : "#1A1A1A",
                  }}
                >
              ...
              <div className="space-y-4">
                {categories.map((category: LeetCodeCategory) => {
                  ...
                  const solvedInCategory = category.problems.filter(
                    (p: LeetCodeProblem) => solvedProblems.has(p.id),
                  ).length;
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                width: `${(solvedInCategory / category.problems.length) * 100}%`,
                              }}
                            />
                          </div>
                          {isExpanded ? (
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      ...
                                      <span
                                        className={`text-sm font-medium ${isSolved ? "line-through text-secondary" : "text-ink"}`}
                                      >
            ...
            {contestInfo?.userContestRanking && (
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    <p className="text-sm text-secondary mb-1">Contests</p>
                    <p className="text-2xl font-serif-bold text-ink">
                  ...
                  <div>
                    <p className="text-sm text-secondary mb-1">Global Rank</p>
                    <p className="text-2xl font-serif-bold text-ink">
                      #
                      {contestInfo.userContestRanking.globalRanking?.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-secondary mb-1">Top %</p>
                    <p className="text-2xl font-serif-bold text-ink">
                      {contestInfo.userContestRanking.topPercentage?.toFixed(2)}
                      %
                    </p>
                  </div>
                </div>
