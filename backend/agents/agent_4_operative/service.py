...
class Agent4Service:
    ...
    def generate_resume(
        ...
        """
        Orchestrates resume generation via the ReACT agent.

        The LLM autonomously decides:
        1. Tailor the resume for the job description
        2. Check ATS score (and re-tailor if too low)
        3. Hunt for recruiter email
        """
        print(f"🚀 [Service] Running ReACT Agent 4 for User {user_id}")
        start_time = time.time()

        from .graph import run_agent4

        agent_result = run_agent4(
        ...
        # Extract structured data from the agent's tool-call history
        pdf_url = ""
        pdf_path = ""
        ats_before = 0
        ats_after = 0
        ...
        for msg in agent_result.get("messages", []):
            # Tool messages contain JSON results from tool executions
            if hasattr(msg, "type") and msg.type == "tool":
                try:
                    payload = json.loads(msg.content)
                except (json.JSONDecodeError, TypeError):
                    continue
                # Extract fields from tailor_resume results
                if "pdf_url" in payload:
                    pdf_url = payload.get("pdf_url", pdf_url)
                    pdf_path = payload.get("pdf_path", pdf_path)
                    ats_before = payload.get("ats_score_before", ats_before)
                    ats_after = payload.get("ats_score_after", ats_after)
                # Extract fields from hunt_recruiter results
                if "email" in payload and "confidence" in payload:
                    recruiter_email = payload.get("email")

        success = bool(pdf_url)
        ...
        if not success:
            raise ValueError("ReACT agent did not produce a tailored resume")
        ...
        return {
            ...
            "message": "Resume generated by ReACT agent",
        }
